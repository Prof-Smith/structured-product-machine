function drawPayoffChart(canvas, currentReturn) {
  ...
  zones.forEach(([a,b,c])=>{ctx.beginPath(); for(let i=0;i<=80;i++){const r=a+(b-a)*i/80; const px=x(r), py=y(structuredNotePayoff(r)); if(i===0)ctx.moveTo(px,py); else ctx.lineTo(px,py);} ctx.strokeStyle=c; ctx.lineWidth=6; ctx.stroke();});
